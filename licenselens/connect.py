"""Native cognis-connect emit for licenselens — forward findings to any platform.

Maps licenselens's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    licenselens ... --format json | licenselens-emit --to stix
    licenselens-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    """Tailor a licenselens record toward the Finding contract before normalize().
