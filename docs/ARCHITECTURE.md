...
- **rules/analyzers** apply the heuristics shipped in `licenselens/core.py`.
- **score** ranks by severity.
- **MCP server** (`licenselens mcp`) exposes `scan` for Cognis.Studio agents.
